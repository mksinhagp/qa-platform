-- Stored Procedure: sp_admin_test_suites_insert
-- Purpose: Create a new admin test suite record for a given run execution
-- Parameters:
--   i_run_execution_id: run_executions.id
--   i_suite_type: 'admin_login', 'booking_lookup', 'registration_lookup',
--                 'admin_edit', 'reporting_screens'
--   i_metadata: optional JSONB metadata (admin context, page config, etc.)
--   i_created_by: operator login or 'system'
-- Returns: o_id, o_status

CREATE OR REPLACE FUNCTION sp_admin_test_suites_insert(

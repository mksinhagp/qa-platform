...
const PaymentTransactionResultPayloadSchema = z.object({
  ...
  execution_id: z.number().int().positive(),
  /** Client-generated UUID for idempotent upserts — retried callbacks
   *  with the same key update the existing row instead of inserting duplicates. */
  ...
  ui_confirmation: z.string().max(2100).nullish(),
...
export async function POST(request: NextRequest) {
  ...
  if (type === 'payment_transaction_result') {
    ...
    try {
      ...
      // Record the payment transaction via an idempotent upsert stored procedure.
      // The idempotency_key (UUID generated by the runner per-operation) is used
      // as a unique constraint — retried callbacks update the existing row instead
      // of creating duplicates.
      const txnResult = await invokeProcWrite('sp_payment_transactions_upsert', {
        i_idempotency_key: p.idempotency_key,
